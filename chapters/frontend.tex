...
Frontend poskytuje samostatné dialógy na import študentov a export dochádzky. Pri importe používateľ najskôr vyberie predmet a potom nahrá CSV súbor. Rozhranie zobrazuje aj náhľad prvých riadkov súboru, čím pomáha odhaliť chybný formát ešte pred samotným odoslaním na backend. Podrobný popis formátu importného súboru (vrátane príkladu) a formátu exportu je uvedený v časti~\ref{sec:backend-export}.
...
Pri exporte si vyučujúci vyberie predmet a frontend následne stiahne výsledný súbor z backendu. Export je určený najmä na ďalšie spracovanie dochádzky mimo samotnej aplikácie (podrobnosti v časti~\ref{sec:backend-export}).
