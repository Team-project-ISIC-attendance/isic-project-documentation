...
\begin{itemize}
  ...
  \item napájanie \textbf{3,3~V} (v prototypovej verzii realizované priamym zdrojom 3,3~V; v produkčnej verzii bude napájanie zabezpečovať Li-Ion batéria s napätím 4,2~V, znižovaným na 3,3~V pomocou napäťového regulátora),
...
\subsubsection{Vývojová varianta s ESP32}
Pre účely vývoja a ladenia firmvéru bola okrem cieľového modulu \textit{ESP-12F} použitá aj vývojová doska \textit{ESP32} (DevKit). Keďže ide o vývojovú dosku s integrovaným USB-UART prevodníkom, programovanie a sériový monitoring sú výrazne pohodlnejšie ako pri holom module ESP-12F. Firmvér je navrhnutý tak, aby bol kompatibilný s oboma platformami; rozdielne sú iba priraďovania pinov.

\begin{table}[h!]
  \centering
  \caption{Prepojenie PN532 s ESP32 DevKit (SPI) -- vývojová varianta}
  \label{tab:pn532-esp32-wiring}
  \begin{tabular}{ll}
    \toprule
    PN532 & ESP32 DevKit \\
    \midrule
    ...
    SS/CS & GPIO5 \\
    IRQ & GPIO4 \\
    RSTO (voliteľné) & voľný GPIO \\
    \bottomrule
  \end{tabular}
\end{table}

ESP32 DevKit bol využívaný výhradne ako vývojová a ladiaca platforma. Produkčný hardvér zostáva navrhnutý pre modul \textit{ESP-12F (ESP8266)}.
...
Na obrázku~\ref{fig:hw-prototype} je zobrazené hardvérové zapojenie prototypu.

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.9\textwidth]{figures/circuit_image.png}
  \caption{Hardvérové zapojenie prototypu}
...
Vstupná udalosť (sken karty) sa publikuje cez \textbf{EventBus} do \texttt{AttendanceService}, kde sa uplatní debounce a prípadné dávkovanie. Následne ju \texttt{MqttService} publikuje do brokeru. Konfigurácia sa spracúva cez \texttt{ConfigService} a stav zariadenia je pravidelne reportovaný službou \texttt{HealthService}. Táto architektúra minimalizuje zásahy do existujúcej funkcionality pri pridávaní novej služby -- nová služba sa zaregistruje v \texttt{App} a prihlási sa na odber relevantných udalostí cez \texttt{EventBus}, pričom ostatné služby zostávajú nezmenené.
...
\begin{itemize}
  \item obojsmerná komunikácia so zariadeniami cez MQTT (prijímanie udalostí, odosielanie konfigurácie a príkazov),
...
\subsubsection{MQTT kontrakt medzi backendom a zariadením}
Každá MQTT téma je scopovaná na konkrétne zariadenie pomocou dvojice konfigurovateľných hodnôt: názvu témy a identifikátora zariadenia.

Tabuľka~\ref{tab:mqtt-sw-hw} sumarizuje správy, ktoré backend posiela zariadeniu (SW\,→\,HW):

\begin{table}[h!]
  \centering
  \caption{MQTT správy backend → zariadenie (SW\,→\,HW)}
  \label{tab:mqtt-sw-hw}
  \begin{tabular}{lp{9cm}}
    \toprule
    Téma (suffix) & Účel \\
    \midrule
    \texttt{config/set} & Nastavenie celej konfigurácie zariadenia \\
    \texttt{config/set/<sekcia>} & Nastavenie jednej sekcie konfigurácie \\
    \texttt{config/get} & Vyžiadanie celej konfigurácie \\
    \texttt{config/get/<sekcia>} & Vyžiadanie jednej sekcie konfigurácie \\
    \texttt{status/request} & Vyžiadanie okamžitého statusu \\
    \texttt{health/request} & Vyžiadanie okamžitého health reportu \\
    \texttt{metrics/request} & Vyžiadanie okamžitého výpisu metrík \\
    \texttt{ota/start} & Spustenie OTA aktualizácie firmvéru \\
    \bottomrule
  \end{tabular}
\end{table}

Tabuľka~\ref{tab:mqtt-hw-sw} sumarizuje správy, ktoré zariadenie posiela backendu (HW\,→\,SW):

\begin{table}[h!]
  \centering
  \caption{MQTT správy zariadenie → backend (HW\,→\,SW)}
  \label{tab:mqtt-hw-sw}
  \begin{tabular}{lp{9cm}}
    \toprule
    Téma (suffix) & Obsah / účel \\
    \midrule
    \texttt{attendance} & Dávka dochádzkových záznamov (JSON pole) \\
    \texttt{status} & Lightweight heartbeat (verzia firmvéru) \\
    \texttt{health} & Zdravotný report zariadenia (heap, Wi-Fi RSSI, uptime\ldots) \\
    \texttt{metrics} & Metriky jednotlivých služieb \\
    \texttt{config} & Odpoveď na \texttt{config/get} (celá konfigurácia) \\
    \texttt{config/<sekcia>} & Odpoveď na \texttt{config/get/<sekcia>} \\
    \texttt{ota/update\_available} & Dostupná nová verzia firmvéru \\
    \texttt{ota/progress} & Priebeh sťahovania OTA (percentá) \\
    \texttt{ota/completed} & OTA úspešne dokončená \\
    \texttt{ota/error} & Chyba počas OTA \\
    \bottomrule
  \end{tabular}
\end{table}
...
Rozlíšenie medzi atribútmi \texttt{scan\_timestamp} a \texttt{created\_at} umožňuje korektné spracovanie situácií, kedy je záznam vytvorený s oneskorením (napríklad pri offline režime zariadenia). Pole \texttt{scan\_timestamp} zodpovedá poľu \texttt{ts} z MQTT payloadu -- absolútnemu Unix času v milisekundách v pásme \textbf{UTC+0}, generovanému priamo na zariadení v momente skenovania. Zariadenie používa UTC+0 ako jednotný štandard, aby vylúčilo problémy s letným časom a časovými pásmami na strane firmvéru; prevod na lokálny čas vykonáva backend podľa nakonfigurovaného časového pásma. Čas zariadenie získava po pripojení k Wi-Fi synchronizáciou cez SNTP (\texttt{pool.ntp.org}, \texttt{time.google.com}). Pokiaľ synchronizácia ešte neprebehla, pole \texttt{ts} má hodnotu~\texttt{0} a backend taký záznam označí ako nevalidný.
...
\begin{itemize}
    ...
    \item \textbf{Normalizácia} -- štruktúra je v tretej normálnej forme (3NF)~\cite{ramakrishnan:dbms}, čo eliminuje redundanciu údajov
...
\begin{figure}[h!]
  ...
  \begin{tabular}{c}
    ...
    $\updownarrow$\\[4pt]
    \fbox{\parbox{0.9\textwidth}{\centering
      \textbf{Wi-Fi + MQTT} (témy scopované na zariadenie: \texttt{\{baseTopic\}/\{deviceId\}/\ldots})
    }}\\[4pt]
    $\updownarrow$\\[4pt]
    \fbox{\parbox{0.9\textwidth}{\centering
    ...
    $\updownarrow$\\[4pt]
    \fbox{\parbox{0.9\textwidth}{\centering
      \textbf{Backend} (MQTT subscriber + publisher, REST API)\\
      spracovanie udalostí, odosielanie konfigurácie a príkazov, REST API pre webové rozhranie
    }}\\[4pt]
    $\downarrow$\\[4pt]
    \fbox{\parbox{0.9\textwidth}{\centering
...
Komunikácia medzi hardvérovým zariadením a backendom je \textbf{obojsmerná} a prebieha výhradne cez MQTT broker. Zariadenie publikuje správy smerom k backendu (HW\,→\,SW): dochádzkové záznamy, pravidelný stav (\texttt{status}), zdravotný report (\texttt{health}), metriky služieb (\texttt{metrics}) a odpovede na konfiguračné požiadavky. Backend naopak posiela príkazy smerom k zariadeniu (SW\,→\,HW): aktualizáciu alebo načítanie konfigurácie (\texttt{config/set}, \texttt{config/get}), vyžiadanie okamžitého reportu (\texttt{status/request}, \texttt{health/request}, \texttt{metrics/request}) a spustenie OTA aktualizácie firmvéru (\texttt{ota/start}).

Hardvérové zariadenie nevykonáva komplexnú aplikačnú logiku (napr.\ mapovanie karty na študenta alebo vyhodnotenie, ku ktorej hodine sken patrí). Jeho primárnou úlohou je \textbf{spoľahlivo zachytiť udalosť} a odoslať ju do komunikačnej vrstvy; okrem toho prijíma konfiguračné príkazy z backendu. Všetky rozhodnutia, validácie a zápisy do databázy sa vykonávajú na serveri, čo zjednodušuje správu viacerých zariadení a umožňuje centralizované zmeny pravidiel.

\section{Ako prebieha zaznamenanie prítomnosti}

Proces zaznamenania prítomnosti začína v momente, keď študent priloží svoju ISIC kartu k NFC čítačke umiestnenej v učebni. Úlohou hardvérového zariadenia je kartu načítať, vykonať základné lokálne kontroly a v prípade platného skenu odoslať udalosť na backend cez MQTT.

\begin{figure}[h!]
  \centering
  \includegraphics[width=\textwidth]{figures/attendance_sequence.png}
  \caption{Sekvenčný diagram procesu skenovania ISIC karty}
  \label{fig:attendance-scan-sequence}
\end{figure}

\subsection{Krok 1: Priloženie a načítanie ISIC karty}

Študent priloží ISIC kartu k NFC čítačke. Zariadenie načíta UID alebo číslo karty a vytvorí lokálnu udalosť skenu. Súčasťou udalosti je aj časová pečiatka, ktorá určuje, kedy bol sken vykonaný. Tento čas je dôležitý pri neskoršom priraďovaní skenu ku konkrétnej vyučovacej hodine.

Hardvérové zariadenie zároveň vykonáva jednoduchú ochranu proti duplicitnému čítaniu. Ak študent priloží tú istú kartu viackrát v krátkom čase, napríklad v intervale dvoch sekúnd, druhé načítanie sa ignoruje. Tým sa zabráni tomu, aby jeden fyzický príchod študenta vytvoril viacero rovnakých udalostí.

\subsection{Krok 2: Lokálna kontrola formátu}

Po načítaní karty zariadenie overí, či načítaná hodnota zodpovedá očakávanému formátu ISIC čísla. Ide o základnú kontrolu, ktorá sa vykonáva ešte priamo na hardvéri.

Ak načítaná hodnota nezodpovedá formátu ISIC čísla, zariadenie udalosť neposiela na backend. Takýto sken sa teda zastaví už na úrovni hardvéru, pretože backend by ho aj tak nevedel priradiť ku študentovi. Tento prístup znižuje počet zbytočných požiadaviek na server a odfiltruje neplatné alebo chybné načítania.

\subsection{Krok 3: Odoslanie udalosti cez MQTT}

Ak karta prejde lokálnymi kontrolami, zariadenie publikuje udalosť do MQTT brokeru. Udalosť obsahuje identifikátor karty, čas skenu a prípadne identifikátor zariadenia alebo učebne. MQTT broker následne doručí túto správu backendu, ktorý je prihlásený na odber príslušnej témy.

Ak backend v danom momente nepracuje alebo nie je dostupný, sken sa nespracuje serverovou logikou. V takom prípade frontend nedostane nový záznam o skene a učiteľ ho neuvidí vo webovom rozhraní.

\subsection{Krok 4: Spracovanie skenu na backende}

Backend po prijatí udalosti vykoná aplikačné overenie. Najprv overí, či karta patrí známemu študentovi. Následne podľa času skenu a zariadenia alebo miestnosti zisťuje, či sa v danej učebni v tomto čase nachádza predmet v rozvrhu.

Ak sa v aktuálnej hodine nenachádza žiadny predmet v rozvrhu, systém nevytvorí záznam dochádzky. Sken sa uloží iba do logu študenta alebo logu skenov. Takýto záznam môže byť neskôr použitý na kontrolu, audit alebo diagnostiku, ale neznamená automatickú prítomnosť na hodine.

Ak predmet v rozvrhu existuje, backend vytvorí alebo aktualizuje záznam dochádzky. Záznam sa uloží do databázy a obsahuje väzbu na študenta, konkrétnu vyučovaciu hodinu, čas skenu a výsledný stav dochádzky, napríklad prítomný.

\subsection{Krok 5: Uloženie výsledku}

Po úspešnom spracovaní sa výsledok uloží do databázy. Databáza tak obsahuje jednak samotné záznamy dochádzky, jednak pomocné logy skenov, ktoré umožňujú spätne analyzovať aj situácie, keď sken neviedol k vytvoreniu dochádzky.

Takto je oddelená jednoduchá úloha hardvéru, ktorou je spoľahlivé načítanie a odoslanie skenu, od rozhodovacej logiky backendu, ktorá určuje, či má byť zo skenu vytvorený dochádzkový záznam.
